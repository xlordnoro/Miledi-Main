...
module.exports = async (client) => {
  const checkVideo2 = async () => {
    const channelId = "UCfAs5tcDB0Y5GqCWaxoaqOQ"; // YouTube channel ID
    const apiKey = process.env.PMG_MUSIC_API_KEY; // Use the YouTube API key from .env file
    ...
    const request = https.request(url, options, async (response) => {
      ...
      response.on("end", async () => {
        try {
          ...
          const rawData = fs.readFileSync(`${__dirname}/../../json/video_PMG_Music.json`);
          ...
          fs.writeFileSync(
            `${__dirname}/../../json/video_PMG_Music.json`,
          ...
          const channel = await guild.channels.fetch("1118665037746282516").catch(console.error);
