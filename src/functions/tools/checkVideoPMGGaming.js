...
module.exports = async (client) => {
  const checkVideo2 = async () => {
    const channelId = "UCIWmiz6utufPO_viWENxyEA"; // YouTube channel ID
    const apiKey = process.env.PMG_GAMING_API_KEY; // Use the YouTube API key from .env file
    ...
    const request = https.request(url, options, async (response) => {
      ...
      response.on("end", async () => {
        try {
          ...
          const rawData = fs.readFileSync(`${__dirname}/../../json/video_PMG_Gaming.json`);
          ...
          fs.writeFileSync(
            `${__dirname}/../../json/video_PMG_Gaming.json`,
          ...
          const channel = await guild.channels.fetch("1055258323324252171").catch(console.error);
